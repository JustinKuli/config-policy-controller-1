	...
	case "Policy":
		...
		for i, tmpl := range tmpls {
			...
			if !cfgPolFound {
				...
				cmd.Println("Ignoring additional ConfigurationPolicy in input")
			}
		}
...
func (d *DryRunner) setupReconciler(
	...
	fake_clientgo_dynamic := dynfake.NewSimpleDynamicClient(scheme.Scheme)
	fake_clientgo_clientset := clientsetfake.NewSimpleClientset()
	watcherReconciler, _ := depclient.NewControllerRuntimeSource()

	dynamicWatcher := depclient.NewWithClients(
		fake_clientgo_dynamic,
		fake_clientgo_clientset.Discovery(),
		watcherReconciler,
	...
	fake_runtime_client := clientfake.NewClientBuilder().
	...
	nsSelReconciler := common.NewNamespaceSelectorReconciler(fake_runtime_client, nsSelUpdatesChan)

	serverVersion := ""

	versionInfo, err := fake_clientgo_clientset.Discovery().ServerVersion()
	if err == nil {
		serverVersion = versionInfo.String()
	}

	rec := ctrl.ConfigurationPolicyReconciler{
		Client:                 fake_runtime_client,
		DecryptionConcurrency:  1,
		DynamicWatcher:         dynamicWatcher,
		Scheme:                 scheme.Scheme,
		Recorder:               record.NewFakeRecorder(8),
		InstanceName:           "policy-cli",
		TargetK8sClient:        fake_clientgo_clientset,
		TargetK8sDynamicClient: fake_clientgo_dynamic,

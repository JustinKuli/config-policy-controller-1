...
var _ = Describe("Test installing an operator from OperatorPolicy", Ordered, func() {
	...
	check := func(
		...
		}, opPolTimeout, 1).Should(Succeed())

		Eventually(func() []v1.Event {
			return utils.GetMatchingEvents(clientManaged, opPolTestNS,
				parentPolicyName, "", expectedEventMsgSnippet, opPolTimeout)
		}, opPolTimeout, 1).ShouldNot(BeEmpty())
	}
	...
	Describe("Testing OperatorGroup behavior when it is not specified in the policy", Ordered, func() {
		...
		It("Should match the OperatorGroup when the extra one is deleted", func() {
		...
		It("Should apply an update to the Subscription", func() {
			...
			check(
				...
				metav1.Condition{
					Type:    "SubscriptionCompliant",
					Status:  metav1.ConditionTrue,
					Reason:  "SubscriptionUpdated",
					Message: "the Subscription was updated to match the policy",
				},
